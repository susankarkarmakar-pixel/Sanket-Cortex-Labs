    ...
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-y-auto bg-bg-main">
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-5 py-6 md:px-8 md:py-8">
        ...
        {!apiKey.trim() ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-950"><div className="flex items-start gap-3"><AlertCircle className="mt-0.5 h-5 w-5 shrink-0" /><div><h2 className="font-semibold">Add your Jules API key</h2><p className="mt-1 leading-6">The Jules key has not loaded. Save it in API Keys, then return here. The key is sent securely through this app’s server route and is never shown in the session view.</p><button type="button" onClick={onOpenSettings} className={`${primaryButton} mt-3`}><Settings className="h-4 w-4" />Open API Keys</button></div></div></div> : !session ? <section className="rounded-2xl border border-border-main/70 bg-surface p-5 shadow-sm">
          ...
          {loadingSources ? <p className="flex items-center gap-2 py-3 text-sm text-text-muted"><Loader2 className="h-4 w-4 animate-spin" />Loading repositories connected to Jules…</p> : sources.length === 0 ? <div className="rounded-xl border border-dashed border-border-main/70 bg-bg-main p-4 text-sm leading-6 text-text-muted"><p className="font-semibold text-text-main">{error ? "Jules could not load repositories." : "No GitHub repositories are connected to this Jules account yet."}</p><p className="mt-1">{error ? "Use the error details below to check the API key or account permission; an empty list here does not necessarily mean no repositories are connected." : "Connect a repository in Jules, install or authorize the Jules GitHub App for that repository, then refresh this list."}</p><div className="mt-3 flex flex-wrap gap-3 text-xs font-semibold"><a href="https://jules.google" target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">Open Jules and connect GitHub</a><a href="https://jules.google/settings" target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">Open Jules API key settings</a><button type="button" onClick={() => void loadSources()} disabled={loadingSources} className="text-accent hover:underline disabled:opacity-50">Refresh repositories</button></div></div> : <form onSubmit={(event) => void createSession(event)} className="space-y-4">
