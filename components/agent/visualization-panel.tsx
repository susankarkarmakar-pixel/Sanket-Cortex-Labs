...
export function VisualizationPanel({ table, sheetTables }: { table: CsvTableSummary | undefined; sheetTables?: SheetTableSummary[] }) {
  ...
  if (!selectedTable?.chartData?.points.length) return <div className="rounded-xl border border-dashed border-border-main bg-surface/60 p-5 text-center"><BarChart3 className="mx-auto h-5 w-5 text-accent" /><p className="mt-2 text-sm font-semibold text-text-main">No chartable numeric data</p><p className="mt-1 text-xs leading-5 text-text-muted">Select a worksheet with at least one numeric column to generate a chart.</p></div>;
  const { column, points } = selectedTable.chartData;
